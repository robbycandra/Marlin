...
void menu_move() {
  ...
  if (
    ...
    if (
      ...
      SUBMENUH31(MSG_MOVE_X, []{ _menu_move_distance(X_AXIS, lcd_move_x); });
      SUBMENUH31(MSG_MOVE_Y, []{ _menu_move_distance(Y_AXIS, lcd_move_y); });
    }
    #if ENABLED(DELTA)
      else
    ...
    STATIC_ITEM_P(PSTR("Axis Not Homed"), SS_CENTER|SS_INVERT);
    ...
    #define SUBMENU_MOVE_E(N) MENU_ITEM_N_P(subedit, N, MSG_MOVE_EN, []{ _menu_move_distance(E_AXIS, []{ lcd_move_e(MenuItemBase::itemIndex); }, MenuItemBase::itemIndex); });
...
void rmenu_motion() {
  ...
  MENU_ITEM_P(submenu22,PSTR("Home Axis"), rmenu_home_axis);
  GCODES_ITEM_P(PSTR("Park Nozzle"), PSTR("G27 P2"));
  GCODES_ITEM(MSG_DISABLE_STEPPERS, PSTR("M84"));
  END_MENU();
}
