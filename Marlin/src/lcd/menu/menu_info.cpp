...
//
  ...
  void menu_info_printer() {
    ...
    START_SCREEN();
    STATIC_ITEM_P(PSTR("Rexyz-" MSG_MARLIN), SS_CENTER|SS_INVERT);       // Marlin
    STATIC_ITEM_P(PSTR(SHORT_BUILD_VERSION));                   // x.x.x-Branch
    STATIC_ITEM_P(PSTR(STRING_DISTRIBUTION_DATE));              // YYYY-MM-DD HH:MM
    STATIC_ITEM_P(PSTR("Rexyz-" MACHINE_NAME));                          // My3DPrinter
...
void menu_info() {
  ...
  #else
    SUBSCREEN6(MSG_INFO_PRINTER_MENU, menu_info_printer);           // Printer Info >
    SUBSCREEN6(MSG_INFO_BOARD_MENU, menu_info_board);               // Board Info >
    #if EXTRUDERS
      SUBSCREEN6(MSG_INFO_THERMISTOR_MENU, menu_info_thermistors);  // Thermistors >
    #endif
  #endif

  #if ENABLED(PRINTCOUNTER)
    SUBSCREEN6(MSG_INFO_STATS_MENU, menu_info_stats);               // Printer Stats >
  #endif
