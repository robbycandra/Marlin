    ...
    // Change filament
      ...
      if (!printer_busy()) {
        // Load filament
          ...
          if (thermalManager.targetTooColdToExtrude(1))
            MENU_ITEM_P(submenu1, msg1, [](){ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 1); });
          else
            MENU_ITEM_P(gcode, msg1, PSTR("M701 T1"));
          #if E_STEPPERS > 2
            PGM_P const msg2 = PSTR(MSG_FILAMENTLOAD " " LCD_STR_E2);
            if (thermalManager.targetTooColdToExtrude(2))
              MENU_ITEM_P(submenu1, msg2, [](){ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 2); });
            else
              MENU_ITEM_P(gcode, msg2, PSTR("M701 T2"));
            #if E_STEPPERS > 3
              PGM_P const msg3 = PSTR(MSG_FILAMENTLOAD " " LCD_STR_E3);
              if (thermalManager.targetTooColdToExtrude(3))
                MENU_ITEM_P(submenu1, msg3, [](){ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 3); });
              else
                MENU_ITEM_P(gcode, msg3, PSTR("M701 T3"));
              #if E_STEPPERS > 4
                PGM_P const msg4 = PSTR(MSG_FILAMENTLOAD " " LCD_STR_E4);
                if (thermalManager.targetTooColdToExtrude(4))
                  MENU_ITEM_P(submenu1, msg4, [](){ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 4); });
                else
                  MENU_ITEM_P(gcode, msg4, PSTR("M701 T4"));
                #if E_STEPPERS > 5
                  PGM_P const msg5 = PSTR(MSG_FILAMENTLOAD " " LCD_STR_E5);
                  if (thermalManager.targetTooColdToExtrude(5))
                    MENU_ITEM_P(submenu1, msg5, [](){ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 5); });
                  else
        ...
        // Unload filament
          ...
          if (thermalManager.targetHotEnoughToExtrude(0))
            ...
            SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E0, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 0); });
          if (thermalManager.targetHotEnoughToExtrude(1))
            GCODES_ITEM(MSG_FILAMENTUNLOAD " " LCD_STR_E1, PSTR("M702 T1"));
          else
            SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E1, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 1); });
          #if E_STEPPERS > 2
            if (thermalManager.targetHotEnoughToExtrude(2))
              GCODES_ITEM(MSG_FILAMENTUNLOAD " " LCD_STR_E2, PSTR("M702 T2"));
            else
              SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E2, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 2); });
            #if E_STEPPERS > 3
              if (thermalManager.targetHotEnoughToExtrude(3))
                GCODES_ITEM(MSG_FILAMENTUNLOAD " " LCD_STR_E3, PSTR("M702 T3"));
              else
                SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E3, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 3); });
              #if E_STEPPERS > 4
                if (thermalManager.targetHotEnoughToExtrude(4))
                  GCODES_ITEM(MSG_FILAMENTUNLOAD " " LCD_STR_E4, PSTR("M702 T4"));
                else
                  SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E4, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 4); });
                #if E_STEPPERS > 5
                  if (thermalManager.targetHotEnoughToExtrude(5))
                    GCODES_ITEM(MSG_FILAMENTUNLOAD " " LCD_STR_E5, PSTR("M702 T5"));
                  else
                    SUBMENU14(MSG_FILAMENTUNLOAD " " LCD_STR_E5, [](){ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 5); });
                #endif // E_STEPPERS > 5
              #endif // E_STEPPERS > 4
            #endif // E_STEPPERS > 3
          #endif // E_STEPPERS > 2
