    ...
    // Change filament
      ...
      if (thermalManager.targetTooColdToExtrude(active_extruder)) {
        if (rexyz_menu_mode == MENUMODE_BASIC)
          MENU_ITEM_P(submenuh21, msg, []{ _menu_temp_filament_op(PAUSE_MODE_CHANGE_FILAMENT, 0); });
        else
          MENU_ITEM_P(submenuh31, msg, []{ _menu_temp_filament_op(PAUSE_MODE_CHANGE_FILAMENT, 0); });
      }
      else
      ...
      for (uint8_t s = 0; s < E_STEPPERS; s++) {
        if (thermalManager.targetTooColdToExtrude(s))
          MENU_ITEM_N_P(submenu14, s, msg, []{ _menu_temp_filament_op(PAUSE_MODE_CHANGE_FILAMENT, MenuItemBase::itemIndex); });
        else {
      ...
      if (!printer_busy()) {
        // Load filament
          ...
          if (thermalManager.targetTooColdToExtrude(active_extruder)) {
            if (rexyz_menu_mode == MENUMODE_BASIC)
              MENU_ITEM_P(submenuh21, msg_load, []{ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 0); });
            else
              MENU_ITEM_P(submenuh31, msg_load, []{ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 0); });
          }
          else
          ...
          for (uint8_t s = 0; s < E_STEPPERS; s++) {
            if (thermalManager.targetTooColdToExtrude(s))
              MENU_ITEM_N_P(submenu14, s, msg_load, []{ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, MenuItemBase::itemIndex); });
            else {
        ...
        // Unload filament
          ...
          for (uint8_t s = 0; s < E_STEPPERS; s++) {
            if (thermalManager.targetTooColdToExtrude(s))
              MENU_ITEM_N_P(submenu14, s, msg_unload, []{ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, MenuItemBase::itemIndex); });
            else {
