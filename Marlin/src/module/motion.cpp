 ...
 */
  ...
  xyze_pos_t current_position = { X_HOME_POS, Y_HOME_POS, MANUAL_Z_HOME_POS };
#else
  xyze_pos_t current_position = { X_HOME_POS, Y_HOME_POS, (float)(Z_HOME_DIR < 0 ? Z_MIN_POS : zv_max_pos) };
#endif
...
void restore_feedrate_and_scaling() {
  ...
  axis_limits_t soft_endstop = {
    ...
    { X_MAX_POS, Y_MAX_POS, (float)zv_max_pos }
  };
  ...
  void update_software_endstops(const AxisEnum axis
      ...
      if (old_tool_index != new_tool_index) {
        ...
        if (axis == Z_AXIS)
          soft_endstop.max[axis] = zv_max_pos + offs;
        else
      ...
      soft_endstop.min[axis] = base_min_pos(axis);
      if (axis == Z_AXIS)
        soft_endstop.max[axis] = zv_max_pos;
      else
